...
class MinigameState extends State
{
	...
	private function changePage(pageInt:Int = 0)
	{
		...
		{
			var minigameWindow:ChallengeWindow = new ChallengeWindow(70 * pageData.col + 10, pageData.row * 60 + 50, pageData.name, pageData.levelName,
				pageData.locked, pageData.textOffsetX, pageData.textOffsetY);
...
class ChallengeWindow extends FlxSpriteGroup
{
	...
	override public function update(elapsed:Float)
	{
		if (!lockedGame)
		{
			...
			{
				game.LawnConfig.curLevel = whatMinigame;
				game.LawnConfig.displayLevel = minigameDisplay;
